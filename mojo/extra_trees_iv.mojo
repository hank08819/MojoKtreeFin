# Mojo Extra Trees REGRESSION — predict Implied Volatility from options data
# d=8: [optionType, strike, spot, DTE, bid, ask, volume, lastPrice]
# Leaf predicts MEAN of training IV values (float32 regression labels)
# Comparison: Mojo trains on 200K samples, sklearn limited to 20K

from std.collections import List
from std.time import perf_counter_ns
from std.pathlib import Path

comptime DIMS=8; comptime NRUNS=3
comptime LEAF=10; comptime MAX_DEPTH=20; comptime MAX_FEAT=3; comptime N_TREES=20

# ── LCG RNG (field of model struct to avoid Copyable tuple issues) ─────────────
struct Rng(Copyable):
    var state:UInt64
...
# ── Regression tree node: leaf stores sum_y and cnt for mean computation ───────
struct TNode(Copyable):
    var feat:Int; var thr:Float32; var left:Int; var right:Int
    var leaf_mean:Float32   # valid only at leaves
    def __init__(out self):
        self.feat=-1; self.thr=Float32(0); self.left=-1; self.right=-1
        self.leaf_mean=Float32(0)
    def copy(self) -> TNode:
        var n=TNode(); n.feat=self.feat; n.thr=self.thr
        n.left=self.left; n.right=self.right; n.leaf_mean=self.leaf_mean
        return n^

# ── Extra Trees Regressor ─────────────────────────────────────────────────────
struct ExTreReg:
    var Xtr:List[Float32]; var Ytr:List[Float32]   # Float32 regression labels
    var Xte:List[Float32]; var Yte:List[Float32]
    var n_tr:Int; var n_te:Int
    var all_nodes:List[TNode]
    var tree_roots:List[Int]
    var idx_buf:List[Int]
    var rng:Rng
    ...
    # ── Build one regression tree (in-place partition on idx_buf) ────────────
    def _build(mut self, lo:Int, hi:Int, depth:Int) -> Int:
        ...
        for i in range(lo,hi):
            if ptr_x2[self.idx_buf[i]*DIMS+bfeat]<=bthr:
                var t=self.idx_buf[s]; self.idx_buf[s]=self.idx_buf[i]
                self.idx_buf[i]=t; s+=1
...
def main() raises:
    var root=String("/Users/henry_han/LHP2/MOJO/extended_benchmark/data/OPTION/")
    print("=== Mojo Extra Trees REGRESSOR   DIMS=",DIMS,"  N_TREES=",N_TREES,"  LEAF=",LEAF," ===")
    print("    Predict Implied Volatility from AAPL options 2016-2020")

    var et=ExTreReg()
    et.load_f32_X(root+"X_train200k.bin", True)
    et.load_f32_X(root+"X_test50k.bin",   False)
    et.load_f32_y(root+"y_train200k.bin", True)
    et.load_f32_y(root+"y_test50k.bin",   False)
    print("  n_train=",et.n_tr,"  n_test=",et.n_te,"  (sklearn uses only 20K)")

    # ── Training ──────────────────────────────────────────────────────────────
    var tb=perf_counter_ns()
    et.build_forest()
    var build_ms=Float64(Int(perf_counter_ns()-tb))/1e6
    print("  Build (train) ms =",build_ms)
    ...
    for run in range(NRUNS):
        ...
        for i in range(et.n_te):
            var pred=et.predict(i)
            var err=Float64(pred)-Float64(et.Yte[i])
            sse+=err*err
            if err>=0.0: sae+=err
            else: sae+=(-err)
    ...
    var rmse=(last_sse/Float64(et.n_te))**0.5
    var mae=last_sae/Float64(et.n_te)
    print("  Inference s      =",infer_s)
    print("  RMSE             =",rmse)
    print("  MAE              =",mae)
    print("  (compare: sklearn ExTraReg 20K in extra_trees_sklearn_iv.py)")
